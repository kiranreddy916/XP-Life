...
BEGIN
  ...
  IF p_completed THEN
    -- Verify if ALL tasks are now checked
    ...
    IF v_total_tasks > 0 AND v_total_tasks = v_completed_tasks THEN
      ...
      IF NOT EXISTS (SELECT 1 FROM activity_logs WHERE user_id = v_user_id AND activity_date = p_client_date AND activity_type = 'all_tasks') THEN
        ...
        RETURN jsonb_build_object('success', true, 'all_completed', true, 'xp_awarded', 50);
      END IF;
    END IF;
  END IF;
  
  RETURN jsonb_build_object('success', true, 'all_completed', false, 'xp_awarded', 0);
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
